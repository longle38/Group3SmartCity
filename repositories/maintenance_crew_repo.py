        ...
        with DatabaseConfig.get_connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    ...
                    "FROM maintenance_crew WHERE crew_id = %s",
                    (crew_id,),
                )
                row = cur.fetchone()
                return MaintenanceCrew.from_row(row) if row else None
    
    def find_all(self, limit=20, offset=0) -> list[MaintenanceCrew]:
        with DatabaseConfig.get_connection() as conn:
            with conn.cursor(row_factory=dict_row) as cur:
                cur.execute(
                    "SELECT crew_id, crew_name "
                    "FROM maintenance_crew ORDER BY crew_id LIMIT %s OFFSET %s",
                    (limit, offset),
                )
                rows = cur.fetchall()
                return [MaintenanceCrew.from_row(row) for row in rows]