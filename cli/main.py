        ...
        while True:
            ...
            try:
                if choice == "1":
                    ...
                    if not inter:
                        print("Intersection not found.", flush=True)
                        continue
                ...
                print(
                    "\nDatabase pool timed out waiting for a connection (often Postgres "
                    "is down, overloaded, or .env points at the wrong host/port).",
                    flush=True,
                )
                print(
                    "Try: verify `pg_isready`, check .env, increase DB_POOL_TIMEOUT if needed.",
                    flush=True,
                )
            except psycopg.Error as exc:
                print(f"\nDatabase error: {exc}", flush=True)
